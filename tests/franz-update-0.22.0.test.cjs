...
test('die Sprechkarte erklärt die automatische Stichwortfunktion zentral statt doppelt', () => {
  const card = read('public/assets/week37-card-print-upgrade.js');
  assert.doesNotMatch(card,/Vollständiger Vorschlag aus Ihren Angaben/);
  assert.match(card,/So funktioniert Ihre Stichwortkarte/);
  assert.match(card,/Automatische Stichwörter neu erstellen/);
  assert.match(card,/Gedächtnisstütze/);
});
...
test('Service Worker erzwingt einen neuen Cache und enthält die neuen Asset-Versionen', () => {
  ...
  assert.match(sw,/franz-lernatelier-v0-22-0-w38-voice-partner/);
  assert.match(sw,/atelier-navigation\.js\?v=20260917-w38nav1/);
  assert.match(sw,/week37-card-print-upgrade\.js\?v=20260917-card6/);
  assert.match(sw,/week38-module\.js\?v=20260917-w38-2/);
});
