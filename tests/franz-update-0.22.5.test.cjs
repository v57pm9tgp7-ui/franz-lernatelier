...
test('Druck übernimmt exakt den aktuell editierten Text und speichert ihn vor dem Drucken', () => {
  ...
  assert.match(card,/function resolvePrintText/);
  assert.match(card,/Object\.prototype\.hasOwnProperty\.call\(options, 'finalText'\)/);
  assert.match(card,/function printNow\(options = \{\}\)/);
  assert.match(card,/const root = createPrintRoot\(currentState, options\)/);
  assert.match(card,/w37-print-final-copy/);
  assert.match(card,/syncPrintBack/);
  assert.match(card,/beforeprint/);
});
...
test('Druckgenerator bevorzugt den explizit übergebenen bearbeiteten Text', () => {
  ...
  global.document = {
    ...
    createElement(){ return {id:'',dataset:{},innerHTML:'',setAttribute(){}}; }
  };
  try {
...
test('Service Worker erzwingt einen neuen Cache und enthält die neuen Asset-Versionen', () => {
  ...
  assert.match(sw,/franz-lernatelier-v0-22-5-w38-print-back-text/);
  assert.match(sw,/atelier-navigation\.js\?v=20260917-w38nav1/);
  assert.match(sw,/week37-card-print-upgrade\.js\?v=20260917-card12/);
  assert.match(sw,/week38-module\.js\?v=20260917-w38-8/);
});


test('Regression: Druckrückseite wird direkt vor dem Drucken mit dem kompletten aktuellen Text befüllt', () => {
  const modulePath = path.join(root,'public/assets/week37-card-print-upgrade.js');
  const oldLocation = global.location;
  const oldDocument = global.document;
  global.location = {pathname:'/module/woche-38/index.html'};
  ...
  const upgrade = require(modulePath);
  global.document = {
    querySelector(selector){ return selector === '[data-w38-final-text]' ? {value:'Bonjour. TEXTE ACTUEL MODIFIÉ. Une phrase ajoutée. Merci.'} : null; },
    querySelectorAll(){ return []; }
  };
  try {
    const back = {textContent:''};
    const fakeRoot = {dataset:{},querySelector(selector){ return selector === '[data-w37-print-final-copy]' ? back : null; }};
    const used = upgrade.syncPrintBack(fakeRoot,{answers:{'w38.finalText':'ancien texte'}},{finalText:'Bonjour. TEXTE ACTUEL MODIFIÉ. Une phrase ajoutée. Merci.'});
    assert.equal(used,'Bonjour. TEXTE ACTUEL MODIFIÉ. Une phrase ajoutée. Merci.');
    assert.equal(back.textContent,used);
    assert.equal(fakeRoot.dataset.printTextLength,String(used.length));
  } finally {
    global.document = oldDocument;
    global.location = oldLocation;
    delete require.cache[require.resolve(modulePath)];
