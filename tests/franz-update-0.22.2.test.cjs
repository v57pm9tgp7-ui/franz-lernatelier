...
test('Übung 6 bietet die Sprechkarte wieder an und druckt den bearbeiteten Finaltext', () => {
  const module = read('public/assets/week38-module.js');
  const card = read('public/assets/week37-card-print-upgrade.js');
  assert.match(module,/Sprechkarte für die Generalprobe/);
  assert.match(module,/data-print/);
  assert.match(module,/Vorne stehen kompakte Stichwörter/);
  assert.match(module,/Rückseite steht Ihr aktuell bearbeiteter Text/);
...
test('Service Worker erzwingt einen neuen Cache und enthält die neuen Asset-Versionen', () => {
  ...
  assert.match(sw,/franz-lernatelier-v0-22-2-w38-print-card/);
  assert.match(sw,/atelier-navigation\.js\?v=20260917-w38nav1/);
  assert.match(sw,/week37-card-print-upgrade\.js\?v=20260917-card8/);
  assert.match(sw,/week38-module\.js\?v=20260917-w38-4/);
});
