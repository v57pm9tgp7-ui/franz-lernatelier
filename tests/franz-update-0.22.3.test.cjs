...
test('Übung 6 zeigt die Druckfunktion sichtbar vor dem Textfeld und druckt den bearbeiteten Finaltext', () => {
  const module = read('public/assets/week38-module.js');
  const card = read('public/assets/week37-card-print-upgrade.js');
  assert.match(module,/Sprechkarte für die Generalprobe/);
  assert.match(module,/data-w38-print-card data-print/);
  assert.ok(module.indexOf('Sprechkarte für die Generalprobe drucken') < module.indexOf('<textarea id=\"w38-final-text\"'), 'Druckfunktion muss vor dem grossen Textfeld sichtbar sein');
  assert.match(module,/api\?\.printNow/);
  assert.match(card,/function printNow\(\)/);
  assert.match(card,/createPrintRoot, printNow/);
  assert.match(module,/vorne kompakte Stichwörter/);
  assert.match(module,/hinten Ihr aktuell bearbeiteter kompletter Text/);
  assert.match(card,/const liveFinal = DISPLAY_WEEK === 38/);
  assert.match(card,/const editedFinal = liveFinal \|\|/);
  assert.match(card,/editedFinal\.split\(\/\\n\+\//);
});

test('Service Worker erzwingt einen neuen Cache und enthält die neuen Asset-Versionen', () => {
  const sw = read('public/service-worker.js');
  assert.match(sw,/franz-lernatelier-v0-22-3-w38-print-visible/);
  assert.match(sw,/atelier-navigation\.js\?v=20260917-w38nav1/);
  assert.match(sw,/week37-card-print-upgrade\.js\?v=20260917-card10/);
  assert.match(sw,/week38-module\.js\?v=20260917-w38-6/);
});
